...
#[test]
fn test_huge_number_of_computed_cells_in_sequence() {
    let mut reactor = Reactor::new();
    let input = reactor.create_input(0);
    let mut outputs = Vec::new();
    let mut old_out = reactor.create_compute(&[CellId::Input(input)], |v| v[0]+1).unwrap();
    outputs.push(old_out);
    for _ in 0..1000000 {
        let new_out = reactor.create_compute(&[CellId::Compute(old_out)], |v| v[0]+1).unwrap();
        outputs.push(new_out);
        old_out = new_out;
    }
    for i in 1..1000000 {
        assert_eq!(reactor.value(CellId::Compute(outputs[i-1])), Some(i as u64));
    }
}
