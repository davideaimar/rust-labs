...
#[test]
fn test_no_texts() {
    assert_eq!(frequency::frequency_chunks(&[], 4), HashMap::new());
}
...
fn test_one_letter() {
    ...
    hm.insert('a', 1);
    assert_eq!(frequency::frequency_chunks(&["a"], 4), hm);
}
...
fn test_case_insensitivity() {
    let mut hm = HashMap::new();
    hm.insert('a', 2);
    assert_eq!(frequency::frequency_chunks(&["aA"], 4), hm);
}
...
fn test_many_empty_lines() {
    let v = vec![""; 1000];
    assert_eq!(frequency::frequency_chunks(&v[..], 4), HashMap::new());
}
...
fn test_many_times_same_text() {
    ...
    hm.insert('b', 1000);
    hm.insert('c', 1000);
    assert_eq!(frequency::frequency_chunks(&v[..], 4), hm);
}

#[test]
#[ignore]
fn test_punctuation_doesnt_count() {
    assert!(!frequency::frequency_chunks(&WILHELMUS, 4).contains_key(&','));
}

#[test]
#[ignore]
fn test_numbers_dont_count() {
    assert!(!frequency::frequency_chunks(&["Testing, 1, 2, 3"], 4).contains_key(&'1'));
}
...
fn test_all_three_anthems_1_worker() {
    ...
    let freqs = frequency::frequency_chunks(&v[..], 1);
...
fn test_all_three_anthems_3_workers() {
    ...
    let freqs = frequency::frequency_chunks(&v[..], 3);
    assert_eq!(freqs.get(&'a'), Some(&49));
    assert_eq!(freqs.get(&'t'), Some(&56));
    assert_eq!(freqs.get(&'ü'), Some(&2));
}
