...
use test::Bencher;
use parallel_letter_frequency::frequency_chunks;

#[bench]
fn bench_tiny_parallel_chunks(b: &mut Bencher) {
    let tiny = &["a"];
    b.iter(|| parallel_letter_frequency::frequency_chunks(tiny, 3));
}

#[bench]
fn bench_tiny_parallel_iter(b: &mut Bencher) {
    let tiny = &["a"];
    b.iter(|| parallel_letter_frequency::frequency_iter(tiny, 3));
}
...
#[bench]
fn bench_small_parallel_chunks(b: &mut Bencher) {
    let texts = all_texts(1);
    b.iter(|| parallel_letter_frequency::frequency_chunks(&texts, 3));
}

#[bench]
fn bench_small_parallel_iter(b: &mut Bencher) {
    let texts = all_texts(1);
    b.iter(|| parallel_letter_frequency::frequency_iter(&texts, 3));
}
...
#[bench]
fn bench_large_parallel_iter(b: &mut Bencher) {
    let texts = all_texts(30);
    b.iter(|| parallel_letter_frequency::frequency_iter(&texts, 3));
}

#[bench]
fn bench_large_parallel_chunks(b: &mut Bencher) {
    let texts = all_texts(30);
    b.iter(|| parallel_letter_frequency::frequency_chunks(&texts, 3));
}
